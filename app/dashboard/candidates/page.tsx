...
const CANDIDATES = [
  ...
  { id:3, initials:'SC', name:'Sherlyn Caoloann', role:'Outbound Appt Setter', exp:'25 yrs', score:88, verdict:'HIRE', status:'Queued', color:'rgba(29,158,117,0.2)', tc:'#1D9E75' },
  ...
  { id:6, initials:'MK', name:'Marcus Kim', role:'Inside Sales Rep', exp:'3 yrs', score:68, verdict:'MAYBE', status:'New', color:'rgba(186,117,23,0.2)', tc:'#EF9F27' },
]

const VS: Record<string,{bg:string,c:string}> = {
  HIRE: { bg:'#E1F5EE', c:'#085041' },
  MAYBE: { bg:'#FAEEDA', c:'#633806' },
  REJECT: { bg:'#FCEBEB', c:'#791F1F' },
}
    ...
    <div className="page-enter">
      ...
      <div className="px-7 py-5">
        <div className="flex gap-2 mb-5">
          {[['all','All'],['HIRE','Hire'],['MAYBE','Maybe'],['Contacted','Contacted']].map(([f,l]) => (
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background:'#111111', border:'1px solid rgba(255,255,255,0.06)' }}>
          {filtered.map((c, i) => (
            <div key={c.id} className="flex items-center gap-4 px-5 py-3.5 group" style={{ borderBottom:i<filtered.length-1?'1px solid rgba(255,255,255,0.04)':'none' }} onMouseEnter={e=>(e.currentTarget.style.background='rgba(255,255,255,0.02)')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
              ...
              <div className="flex-1 min-w-0"><div className="text-sm font-medium truncate" style={{ color:'#F0EFEA' }}>{c.name}</div><div className="text-xs" style={{ color:'#4A4845' }}>{c.role} · {c.exp}</div></div>
