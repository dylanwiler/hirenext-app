...
const CANDIDATES = [
  ...
  { id:3, initials:'SC', name:'Sherlyn Caolann', role:'Outbound Appt Setter', exp:'25 yrs', score:88, verdict:'HIRE', status:'Queued', color:'rgba(29,158,117,0.2)', tc:'#1D9E75' },
  ...
  { id:8, initials:'MK', name:'Marcus Kim', role:'Inside Sales Rep', exp:'3 yrs', score:68, verdict:'MAYBE', status:'New', color:'rgba(186,117,23,0.2)', tc:'#EF9F27' },
]

const VS: Record<string,{bg:string,c:string}> = {
  HIRE:{bg:'#E1F5EE',c:'#085041'},
  MAYBE:{bg:'#FAEEDA',c:'#633806'},
  REJECT:{bo:'#FCEBEB',c:'#791F1F'},
}
  ...
  return (
    <div className="page-enter">
      <div className="flex items-center justify-between px-7 py-5" style={{ borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
        <div><h1 className="font-display font-extrabold text-xl" style={{ color:'#F0EFEA' }}>Candidate Pipeline</h1><p className="text-sm mt-0.5" style={{ color:'#4A4845' }}>553 total · 48 hire-tier · 2 contacted</p></div>
        <div className="flex items-center gap-3">
          ...
          <button className="btn-primary text-sm"><ArrowUpRight size={13} /> Run scan</button>
        </div>
      </div>
      <div className="px-7 py-5">
        <div className="flex gap-2 mb-5">
          {[['all','All'],['HIRE','Hire'],['MAYBE','Maybe'],['Contacted','Contacted'],['Interviewed','Interviewed']].map(([f,l]) => (
            <button key={f} onClick={() => setFilter(f)} className="text-xs px-4 py-2 rounded-xl transition-all" style={{ background:filter===f?'rgba(29,158,117,0.15)':'rgba(255,255,255,0.03)', border:`1px solid ${filter===f?'rgba(29,158,117,0.4)':'#2A2A2A'}`, color:filter===f?'#F0EFEA':'#4A4845' }}>{l}</button>
          ))}
        </div>
        <div className="rounded-2xl overflow-hidden" style={{ background:'#111111', border:'1px solid rgba(255,255,255,0.06)' }}>
          {filtered.map((c, i) => (
            <div key={c.id} className="flex items-center gap-4 px-5 py-3.5 group" style={{ borderBottom:i<filtered.length-1?'1px solid rgba(255,255,255,0.04)':'none' }} onMouseEnter={e=>(e.currentTarget.style.background='rgba(255,255,255,0.02)')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
              <div className="w8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ background:c.color, color:c.tc }}>{c.initials}</div>
              ...
              {VS[c.verdict] && <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background:VS[c.verdict].bg, color:VS[c.verdict].c }}>{c.verdict}</span>}
        ...
        <p className="mt-4 text-xs text-center" style={{ color: '#3A3A3A' }}>Showing {filtered.length} of 553 candidates</p>
      </div>
    </div>
  )
}
