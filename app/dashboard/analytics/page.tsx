...
import { BarChart2, TrendingUp, Users, Mic, DollarSign, Zap } from 'lucide-react'

const WEEKLY = [
  { day:'Mon', scanned:45, scored:30, hire:12, maybe:10, reject:8 },
  { day:'Tue', scanned:68, scored:50, hire:18, maybe:15, reject:17 },
  { day:'Wed', scanned:72, scored:58, hire:20, maybe:18, reject:20 },
  { day:'Thu', scanned:35, scored:25, hire:8, maybe:9, reject:8 },
  { day:'Fri', scanned:55, scored:40, hire:15, maybe:12, reject:13 },
  { day:'Sat', scanned:20, scored:15, hire:5, maybe:5, reject:5 },
  { day:'Sun', scanned:0, scored:0, hire:0, maybe:0, reject:0 },
]
...
export default function AnalyticsPage() {
  const maxVal = Math.max...WEEKLY.map(w => w.scanned)
  return (
          ...
          <div className="flex items-end gap-2 h-32">
            {WEEKLY.map((w, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2">
                <div className="width[60px] rounded-t-lg transition-all relative overflow-hidden" style={{ height:`${Math.max(2,(w.scanned/maxVal)*100)}%`, background:'#1D9E75', opacity:w.scanned>0?1:0.2, minHeight:'2px' }} />
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="rounded-2xl p-5" style={{ background:'#111111', border:'1px solid rgba(255,255,255,0.06)' }}>
            ...
            {[['Hire', 48, '#1D9E75'],['Maybe', 202, '#BA7517'],['Reject', 303, '#E24B4A']].map(([l,v,c]) => (
              <div key={l} className="mb-3">
                <div className="flex items-center justify-between text-xs mb-1"><span style={{ color:'#8A8880' }}>{lowerCase}</span><span className="font-bold" style={{ color:c }}>{v}</span></div>
                <div className="h-1.5 rounded-full overflow-hidden" style={{ background:'rgba(255,255,255,0.06)' }}><div className="h-full rounded-full" style={{ width:`${(v/553)*100}%`, background:c }} /></div>
              </div>
            ))}
          </div>
          <div className="rounded-2xl p-5 col-span-2" style={{ background:'#111111', border:'1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[{ l:'Per recruiter', v:'$4,000', sub:'Typical fee' },{l:'Hire Next AI', v:'$600', sub:'Score our plan' },{ l:'You saved', v:'$3,400', sub:'12 hires in', high:true }].map((s, i) => (
