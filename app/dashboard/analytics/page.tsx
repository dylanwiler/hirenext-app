...
export default function AnalyticsPage() {
  const maxVal = WEEKLY.reduce((max, w) => Math.max(max, w.scanned), 1)
  return (
          ...
          <div className="flex items-end gap-2 h-32">
            {WEEKLY.map((w, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full rounded-t-lg" style={{ height:`${Math.max(2,(w.scanned/maxVal)*100)}%`, background:'#1D9E75', minHeight:'2px', opacity:w.scanned>0?1:0.2 }} />
          ...
          <div className="rounded-2xl p-5" style={{ background:'#111111', border:'1px solid rgba(255,255,255,0.06)' }}>
            ...
            {[['Hire',48,'#1D9E75'],['Maybe',202,'#BA7517'],['Reject',303,'#E24B4A']].map(([l,v,c]) => (
              <div key={String(l)} className="mb-3">
                <div className="flex justify-between text-xs mb-1"><span style={{ color:'#8A8880' }}>{l}</span><span className="font-bold" style={{ color:String(c) }}>{v}</span></div>
