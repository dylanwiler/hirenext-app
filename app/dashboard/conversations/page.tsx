...
const CONVS = [
  { id:'justin', av:'JG', color:'rgba(29,158,117,0.2)', tc:'#1D9E75', name:'Justin Griffis', preview:'Message sent · Awaiting reply', time:'12:01 AM', score:91, platform:'ZipRecruiter',
  ...
  { id:'tara', av:'TG', color:'rgba(29,158,117,0.2)', tc:'#1D9E75', name:'Tara Gaulin', preview:'Message sent · Awaiting reply', time:'12:02 AM', score:90, platform:'ZipRecruiter',
  ...
  { id:'wendy', av:'WH', color:'rgba(29,158,117,0.2)', tc:'#1D9E75', name:'Wendy Hawthorne', preview:'Outreach queued', time:'Queued', score:87, platform:'ZipRecruiter', msgs:[]},
]
        ...
        <div className="flex-1 overflow-y-auto">
          {convs.map(c => (
            <div key={c.id} onClick={() => setActive(c.id)} className="flex items-start gap-3 px-4 py-3.5 cursor-pointer transition-all" style={{ background:active===c.id?'rgba(255,255,255,0.03)':'transparent', borderBottom:'1px solid rgba(255,255,255,0.04)', borderLeft:`4px solid ${active===c.id?'#1D9E75':'transparent'}` }}>
              <div className="w8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background:c.color,color:c.tc }}>{c.av}</div>
